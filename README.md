...
* **remote** - дочернее приложение или стрим. Загружается по сети и встраивается в родительский контейнер.

## Запуск

При локальном запуске строго соблюдайте последовательность — сперва remote, после host. Это нужно для того, чтобы host мог найти ваши remote's.

### Удаленное приложение

1. Перейдите в директорию `tutorial-video`
2. Установите зависимомти `npm install`
